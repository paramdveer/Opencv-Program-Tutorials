...
#ap.add_argument("-r", "--ref", required=True,help="path to the Refference image")
ap.add_argument("-i", "--image", required=True,help="path to the test  image")
args = vars(ap.parse_args())
#----------------------------------Loading The Refrence Image and Alligning It-------------------
Exam=ImageAlignment.alignImages(cv2.imread(args["image"]),cv2.imread("/home/paramveer/Downloads/images/page3.jpeg"))
...
for (q, i) in enumerate(np.arange(0, len(questionCnts), 4)):
    ...
    for (j, c) in enumerate(cnts):
        ...
        # bubble area
        mask2=cv2.bitwise_and(thresh,mask)
        cv2.imshow("Mask2",mask2)
        mask = cv2.bitwise_and(thresh, thresh, mask=mask)
        total = cv2.countNonZero(mask)
        cv2.imshow("Counters with Mask and Mark",mask)
        print('total White pixels =',total)
        cv2.waitKey(0)
...
Exam=om.crop("/home/paramveer/Downloads/images/test.jpeg")
